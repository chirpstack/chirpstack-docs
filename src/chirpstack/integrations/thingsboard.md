...
If configured, the ThingsBoard integration will send device attributes and
telemetry to the configured [ThingsBoard](https://thingsboard.io/) instance.
...
[live device event-log](../use/event-logging.md) feature. Decoded payload data
will be available under the `object` key in the JSON object.

ThingsBoard will generate a _Access Token_ per device. This token must be
configured as a [device variable](../use/devices.md) in ChirpStack. The variable
must be named **ThingsBoardAccessToken**.

## Attributes

For each event, ChirpStack will update the ThingsBoard device with the following
attributes:
...
In case any [device tags](../use/devices.md) are configured for the device in
ChirpStack, these will also be added to the attributes.
...
device-status requests can be configured through the
[Device Profile](../use/device-profiles.md).
