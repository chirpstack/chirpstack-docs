...
apply some changes. The **Full** image will setup the PostgreSQL database on its
...
After your gateway has booted, it will use DHCP to obtain an IP address. Many
internet routers provide a web-interface where you will find the IP addresses of
connected devices.

#### Wi-Fi

If your gateway has Wi-Fi support, then the ChirpStack Gateway OS will create an
access-point to which you can connect with the name `ChirpStackAP-XXXXXX` and
...
- It is recommended to reconfigure the gateway to connect to your Wi-Fi network,
  or to disable the access-point mode if the gateway is connected using
  ethernet.
...
Your browser might show you a warning about a self-signed certificate. Click
proceed.
...
2. Click the **Scan** button, after a couple of seconds this will show the
...
4. In the Joining Network form, enter the following values and click **Submit**:
   ...
   - _Create / Assign firewall-zone_: Select the dropdown item with the green
     background.
5. Click **Save**
6. Click **Save & Apply**

After your gateway has connected to the Wi-Fi network, you need to obtain the IP
address again and re-connect. Please see the _Connecting_ steps.

## Flash concentrator firmware (optional)

In case you have a USB based concentrator module (USB based SX1302/3 module or
2.4GHz module), you must flash the MCU firmware of the module before it can be
used.
...
- SX1301, SX1302/SX1303, 2.4GHz tabs (matching the _Enabled chipset_)
  ...
  - _Channel-plan_: Select the channel-plan. This will show an error if the
    channel-plan is not supported by the gateway or if the channel-plan has not
...
- **Save & Apply** will automatically configure the ChirpStack MQTT Forwarder
  MQTT prefix.
- If you are using the **Full** image, then this will also enable the region in
  ChirpStack.
...
**Note:** If you are using the **Full** image, then ChirpStack MQTT Forwarder is
already configured to forward to the local ChirpStack Network Server instance.
...
- _Server_: The address of the MQTT broker, e.g. `tcp://example.com:1883` or
  `ssl://example.com:8883` (TLS).

Depending the configuration of the MQTT broker, you might also want to edit the
_Username_ / _Password_ fields or _CA certificate_ / _TLS certificate_ and _TLS
key-file_ fields.
...
The ChirpStack UDP Forwarder makes it possible to connect your gateway to one or
multiple Network Servers compatible with the Semtech UDP Packet-Forwarder
...
To configure the ChirpStack UDP Forwarder, click **ChirpStack** and then **UDP
Forwarder** in the left menu.
...
- _Server_: The hostname or IP address + port of the server, e.g.
  `example:1700`.
...
**Full** image (see [Image types](./image-types.md)), then you are ready to use
the ChirpStack LoRaWAN Network Server.

To open ChirpStack, click **Applications** in the left menu, and then click the
**ChirpStack** logo. This will open the ChirpStack web-interface in a new
window.

Please refer to the [Connecting a device](../guides/connect-device.md) guide for
