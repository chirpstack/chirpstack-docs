...
**Note:** These are custom firmware images that are not supported by RAK and
...
This replaces the WisGateOS2 firmware of the gateway with the ChirpStack
GatewayOS OS.
...
You must set the `compat_version` parameter to `2.1.4`, or else the upgrade will
fail with:

> The device is supported, but this image is incompatible for sysupgrade based
> on the image version (1.0->2.1.4)
...
Once the WisGateOS2 image is restored, you should see the WisGateOS2
web-interface which will guide you through the gateway configuration. Once
setup, you should update the firmware to the latest WisGateOS2 version! You can
download the latest WisGateOS2 firmware image from the
[RAK Download Center](https://downloads.rakwireless.com/#LoRa/WisGateOS2/).

### RAK7391

For instructions on how to flash an image to the Compute Module 4, please refer
to
