...
[Vagrant](https://www.vagrantup.com/) is a tool for automating the creation of
virtual machines. It can integrate with Ansible so that it not only create the
VM for you, but also will install and configure ChirpStack.

After following the instructions documented in the
...
this can also be used for testing before doing a remote deployment, e.g. when
